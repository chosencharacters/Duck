...
class Ctrl
{
	...
	public static function update()
	{
		...
		for (c in 1...2)
		{
			...
			menuLeft[c] = jnone[c];
			menuRight[c] = juse[c];
			menuConfirm[c] = jjump[c] && !Main.REVERSE_MENU_CONTROLS || jnone[c] && Main.REVERSE_MENU_CONTROLS;
			menuBack[c] = jjump[c] && Main.REVERSE_MENU_CONTROLS || jnone[c] && !Main.REVERSE_MENU_CONTROLS;

			anyB[c] = up[c] || down[c] || left[c] || right[c] || jump[c] || attack[c] || none[c] || use[c] || pause[c] || map[c] || reset[c] || jroll[c];
	...
	{ // gamepad controls
		...
		for (p in 1...3)
		{
			...
			if (gp != null && FlxG.gamepads.getByID(gp.id) != null)
			{
				...
				menuConfirm[p] = jjump[p] && !Main.REVERSE_MENU_CONTROLS || jnone[p] && Main.REVERSE_MENU_CONTROLS;
				menuBack[p] = jjump[p] && Main.REVERSE_MENU_CONTROLS || jattack[p] && !Main.REVERSE_MENU_CONTROLS;
				...
				anyB[p] = up[p] || down[p] || left[p] || right[p] || jump[p] || attack[p] || none[p] || use[p] || pause[p] || map[p] || reset[p] || jroll[p];
